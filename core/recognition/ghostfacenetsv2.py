...
class ModifiedGDC(nn.Module):
    def __init__(self, image_size, in_chs, num_classes, dropout, emb=512): # dropout implementation is in the original code but not in the paper
        ...
        self.conv_dw = nn.Conv2d(in_chs, in_chs, kernel_size=(7), groups=in_chs, bias=False)
...
class GhostFaceNetsV2(nn.Module):
    def __init__(self, cfgs=None, image_size=256, num_classes=0, width=1.0, channels=3, dropout=0.2, block=GhostBottleneckV2,
                 add_pointwise_conv=False, bn_momentum=0.9, bn_epsilon=1e-5, init_kaiming=True,fp16=False, args=None):
        ...
        self.conv_stem = nn.Conv2d(channels, output_channel, 3, 1, 1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.classifier(x.float() if self.fp16 else x)
...
if __name__ == "__main__":
    ...
    # time.sleep(10)
    for i in range(10):
        img = torch.randn(3, 3, IMAGE_SIZE, IMAGE_SIZE).cpu()
        x = time.time()
        size = model(img)
        y = time.time()
        print(y - x)
